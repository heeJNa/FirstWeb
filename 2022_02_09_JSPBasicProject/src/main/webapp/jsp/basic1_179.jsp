...
<%--
		...
		=> 응답 정보 (contentType => html,xml,json) => 브라우저에 알려준
		=> reDirect : 화면 이동
		=> 데이터 보내기 전에 먼저보내는 데이터 : header
			header : 다운로드창 (파일명 등)
		=> setHeader(), sendRedirect("이동할 파일") : 화면이동
--%>
<!DOCTYPE html>
<html>
<head>
				...
				<tr>
					...
					<td width=85%>
						<ul>
							...
							<li>헤더 정보: 실제데이터를 보내기 전에 먼저 보내는 데이터가 있는 경우(다운로드)</li>
							<li>서버 파일 이동: reDirect</li>
						</ul>
					</td>
				</tr>
				<tr>
					<td width=15%>주요 메소드</td>
					<td width=85%>
						<ul>
							<li>응답정보: setContentType("text/html; charset=UTF-8")</li>
							<li>헤더정보: setHeader("Content-Disposition","attachment"; filename=..")
