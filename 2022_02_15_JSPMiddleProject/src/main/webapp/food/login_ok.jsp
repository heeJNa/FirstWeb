...
<%
    ...
    * */
    ...
    // 요청처리 => 오라클안에 있는지 확인 => DAO연결 => AJAX(기존 화면에서 바로 출력),Jquery
    // 2. 오라클에서 결과값을 받는다.
...
<%
    ...
    // 로그인
    /*
        프로그램에 필요한 개인의 일부를 서버에 저장할 목적 => 세션
        세션(session) : 저장이 되면 모든 JSP에서 사용이 가능
        === 브라우저 종료 / 로그아웃 => 세션에 저장된 기록이 없어진다.
        === 저장하는 시간 (기본 default : 30분)
        세션 => JSP제공하는 메소드
        1) 저장
            setAttribute(ket,value) => cookie는 값(문자열
                                    == session은 값 (Object)
        2) 데이터 읽기
            Object getAttribute(키)
        3) 저장 기간 설정
           setMaxInactiveInterval(초단위); => 1800초 = 30분
        4) 저장정보 일부 삭제
            removeAttribute(키)
        5) 전체 삭제(로그아웃)
            invalidate()
