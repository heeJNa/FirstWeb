...
<%--
        1. 내장 객체 : 미리 생성된 객체(JSP)
            ...
            -------------------------------- 위 내용들은 Spring까지 사용이 된다.
        2. 지시자 : page, taglib (MVC,Spring => 사용)
        3. JSP액션태그 : <jsp:include> => 조립식 프로그램, 분업이 가능
        ------------------------------------------------------------------------------
        ...
        웹 프로그램의 패턴
        model 1 => (JSP => JSP) => 장점 : 사용하기 쉽다, 간단한 Java사용만 하면 된다. (제어문, 연산자, 변수선언, 데이터베이스 연동)
                               => 단점 : 확정성(x), 재사용(x)
                                        분업이 불가능 (혼자 전체 사이트를 제작)
                                        HTML/CSS/Java/JavaScript
          mv   => Model(자바) => VO, DAO, 요청처리
                  View => 순수하게 태그형으로 출력
                  단점 : JSP에서 클래스 메소드를 호출해서 사용 (완전하게 Java/HTML을 분리하지 못했다)
        model 2(MVC) => Model : 자바 ==> Back => 확장성, 재사용, 수정
                       View  : HTML/CSS ==> Front
                       Controller : 연결
    =============================================================================================================
     ...
     5. MVC
        ...
        JSP(View) ==> Controller ==> Model ==> Controller ==> JSP(View)
--%>
<!DOCTYPE html>
<html>
<head>
